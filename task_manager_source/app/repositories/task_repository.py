    ...
    view: str = "all",
        ...
        if view == "today":
            ...
            statement = statement.where(
                ...
                # Task must have started on or before today
                TaskEntity.planned_start_date
                < tomorrow_start,

                # Task must still be active today
                or_(
                    TaskEntity.planned_end_date
                    >= today_start,

                    # If no planned end date exists,
                    # consider it active after it starts
                    TaskEntity.planned_end_date.is_(None),
                ),
            )
    ...
    task_id: int,
        ...
        while pending_parent_ids:
            ...
            statement = (
                select(TaskEntity)
                .where(
                    TaskEntity.parent_task_id == parent_id
                )
                .order_by(TaskEntity.task_id)
            )
    ...
    def delete_tasks_by_ids(
        ...
        *,
        ...
        statement = (
            delete(TaskEntity)
            .where(
                TaskEntity.task_id.in_(task_ids)
            )
        )
