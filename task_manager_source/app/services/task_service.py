...
class TaskService:
    ...
    def can_delete_hierarchy(
        self,
        task_id: int,
    ) -> tuple[bool, str | None]:
        root_task = self.repository.get_by_id(task_id)
        ...
        hierarchy = [
            root_task,
            *self.repository.get_all_descendants(
                task_id
            ),
        ]
        ...
        if invalid_tasks:
            ...
            return (
                False,
                (
                    "Delete is not allowed because these tasks "
                    f"are not in a final status: {task_numbers}."
                ),
            )
    ...
    def delete_hierarchy(
        ...
        *,
        ...
        can_delete, reason = self.can_delete_hierarchy(
            task_id
        )

        if not can_delete:
            raise ValueError(
                reason or "Task hierarchy cannot be deleted."
            )

        descendants = (
            self.repository.get_all_descendants(task_id)
        )
        ...
        if parent_task_id != 0:
            parent_task = self.repository.get_by_id(
                parent_task_id
            )

        activities = (
            self.activity_service.repository.get_by_task_ids(
                task_ids
            )
        )

        self.delete_archive_service.append_deleted_hierarchy(
            root_task=root_task,
            tasks=hierarchy,
        ...
        try:
            ...
            self.repository.delete_tasks_by_ids(
                task_ids,
                commit=False,
            )

            if parent_task is not None:
                self.activity_service.repository.create_without_commit(
                    TaskActivityCreate(
                        task_id=parent_task.task_id,
                        activity_type=ActivityType.SYSTEM,
                        title="Subtask Hierarchy Deleted",
                        message=(
                            f"Task {root_task.task_number} "
                            f"(Task ID {root_task.task_id}) and "
                            f"{len(descendants)} descendant task(s) "
        ...
        return {
            "deleted_root_task_id": task_id,
            "deleted_root_task_number": (
                root_task.task_number
            ),
            "deleted_task_count": len(hierarchy),
            "parent_task_id": parent_task_id,
        }
    ...
    def _status_value(
        ...
        if normalized_value.startswith("taskstatus."):
